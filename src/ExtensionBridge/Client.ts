...
const stubEffectModule: VSBloomEffectModule = {
	Start: (_configResolver: EffectConfigResolver) => {},
	Stop: () => {},
};
...
class VSBloomClient implements IVSBloomClient {
	...
	private set windowId(value: string) {
		const isInitialWindowIdAssignment =
			this._windowId === 'WINDOW_ID_NOT_ASSIGNED_FOR_SOME_REASON';
	...
	private CreateTrustedTypesPolicy(): TrustedTypePolicy | null {
		...
		try {
			...
			this.Log(
				'warn',
				'Failed to create a Trusted Types policy for use by the VSBloom Client',
				{ error: String(error) },
			);
			return null;
		}
		...
		try {
			...
			this.ws.onclose = (event: CloseEvent) => {
				...
				if (event.code === 4001) {
					...
					this.Log(
						'error',
						'Connection rejected: Invalid auth token(?) - A re-patch is likely required',
					);
					return;
				}
				...
				if (
					...
					event.target.readyState === WebSocket.CLOSED
				) {
					this.Log(
						'debug',
						'WebSocket connection closed with an internal error code, server likely still initializing or not available; scheduling a reconnect attempt',
						{ event },
					);
			...
			this.ws.onerror = (error: Event) => {
				...
				if (
					...
					error.target.readyState === WebSocket.CLOSED &&
					!('code' in error && error.code === 1006)
				) {
					this.Log('debug', 'An error occurred with the WebSocket while it was closed', {
						errorMessage: String(error),
					});
					return;
				}
				this.Log('error', 'An internal error occurred with the WebSocket', {
					errorMessage: String(error),
				});
			};
		} catch (error) {
	...
	private ScheduleReconnect(): void {
		...
		this.Log(
			'info',
			`Scheduled reconnection to VSBloom Extension in ${delay}ms (attempt ${this.reconnectAttempts})`,
		);
	...
	private HandleMessageFromExtension(message: ExtensionToClientMessage): void {
		switch (message.type) {
			case 'enable-effect':
				this.EnableClientEffect(
					message.effectName,
					message.effectDisplayName,
					message.js,
					message.css,
				);
				break;
	 ...
	 */
	private async EnableClientEffect(
		effectName: string,
		effectDisplayName: string,
		js?: string,
		css?: string,
	): Promise<void> {
		const effectHandle: LoadedVSBloomEffectHandle = await this.GetLoadedEffectHandle(
			effectName,
			js,
			css,
		);
		...
		if (effectHandle.isEnabled) {
			this.Log(
				'error',
				`Effect "${effectName}" is already enabled during call to EnableClientEffect(?) - this is indicative of a synchronization issue`,
			);
			return;
		}
		...
		try {
			...
			const startResult = effectHandle.module.Start(
				new EffectConfigResolver(effectDisplayName),
			);
			...
			this.Log(
				'error',
				`An error occurred while attempting to start effect "${effectName}": This may lead to undefined behavior or memory leaks!`,
				{ errorMessage: String(error) },
			);
		}
	}
	 ...
	 */
	private async GetLoadedEffectHandle(
		effectName: string,
		js?: string,
		css?: string,
	): Promise<LoadedVSBloomEffectHandle> {
		const jsHash = js ? this.NaiveHash(js) : undefined;
		const cssHash = css ? this.NaiveHash(css) : undefined;

		const preExistingHandle: LoadedVSBloomEffectHandle | undefined =
			this.effectHandles.get(effectName);
		if (preExistingHandle) {
			//ensure the handle's JS and CSS match the new ones if they're provided

			if (
				jsHash !== preExistingHandle.jsSrcHash ||
				cssHash !== preExistingHandle.cssSrcHash
			) {
		...
		try {
			...
			if (
				typeof effectModule.Start !== 'function' ||
				typeof effectModule.Stop !== 'function'
			) {
				...
				this.Log(
					'error',
					`Effect module for effect name "${effectName}" does not export the required interface`,
					{ effectModule },
				);
				throw new Error(
					`Effect module for effect name "${effectName}" does not export the required interface`,
			...
			this.Log('error', `Failed to load effect module for effect with name "${effectName}"`, {
				errorMessage: String(error),
			});
	...
	private async ReloadClientEffect(effectName: string): Promise<void> {
		const effectHandle: LoadedVSBloomEffectHandle | undefined =
			this.effectHandles.get(effectName);
		if (!effectHandle) {
			this.Log(
				'error',
				`ReloadEffectModule called with effect name "${effectName}" but no effect by this name is currently loaded on the client`,
			);
			return;
		}
		...
		try {
			...
			effectHandle.isEnabled = true;
			this.Log('debug', `Reloaded effect "${effectName}" successfully`, {
				effectHandle,
				stopResult,
				startResult,
			});
			...
			this.Log(
				'error',
				`An error occurred while attempting to reload effect "${effectName}": This may lead to undefined behavior or memory leaks!`,
				{ errorMessage: String(error) },
			);
		}
	}
	...
	private async StopClientEffect(effectName: string): Promise<void> {
		...
		if (!effectHandle) {
			this.Log(
				'error',
				`StopClientEffect called with effect name "${effectName}" but no effect by this name is currently loaded on the client`,
			);
			return;
		}
		...
		try {
			...
			this.Log(
				'error',
				`An error occurred while attempting to stop effect "${effectName}": This may lead to undefined behavior or memory leaks!`,
				{ errorMessage: String(error) },
			);
		} finally {
