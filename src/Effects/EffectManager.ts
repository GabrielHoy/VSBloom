...
export class EffectManager implements vscode.Disposable {
	...
	private HandleExtensionConfigsChanged(): void {
		...
		for (const [effectName, effectConfig] of this.staticEffectConfigs.entries()) {
			...
			const effectEnabledPropPath = GetInternalPathForEffectProperty(
				effectConfig.effectDisplayName,
				'enabled',
			);
			const hasEnabledProperty = DoesExtensionConfigValueExist(
				currentCfg,
				effectEnabledPropPath,
			);
			if (!hasEnabledProperty) {
				...
				this.Log(
					'error',
					`HandleExtensionConfigsChanged - Effect "${effectName}" does not have an 'enabled' property defined in its configuration: ${JSON.stringify(effectConfig, null, 2)}`,
				);
				continue;
			}

			const shouldEffectBeEnabled: boolean = GetExtensionConfigValue(
				currentCfg,
				effectEnabledPropPath,
				false,
			);
	...
	private LoadStaticEffectConfigs(): void {
		...
		if (!fs.existsSync(effectConfigsDir)) {
			this.Log(
				'error',
				`LoadStaticEffectConfigs called but the effects configuration directory "${effectConfigsDir}" does not exist`,
			);
			return;
		}
		if (!fs.statSync(effectConfigsDir).isDirectory()) {
			this.Log(
				'error',
				`LoadStaticEffectConfigs called but the effects configuration directory "${effectConfigsDir}" is not a directory`,
			);
			return;
		}
		const staticEffectSubdirs = fs
			.readdirSync(effectConfigsDir)
			.filter((dir) => fs.statSync(path.join(effectConfigsDir, dir)).isDirectory());
		for (const staticEffectDir of staticEffectSubdirs) {
			const effectJSONPath = path.join(
				effectConfigsDir,
				staticEffectDir,
				`${staticEffectDir}.jsonc`,
			);
			if (!fs.existsSync(effectJSONPath)) {
				this.Log(
					'error',
					`LoadStaticEffectConfigs - Effect "${staticEffectDir}" does not have a configuration JSON file at "${effectJSONPath}"`,
				);
				continue;
			}

			try {
				const effectConfig = jsonc.parse(
					fs.readFileSync(effectJSONPath, 'utf8'),
				) as EffectConfiguration;
				this.staticEffectConfigs.set(staticEffectDir, effectConfig);
			} catch (error) {
				this.Log(
					'error',
					`LoadStaticEffectConfigs - Error parsing effect configuration file "${effectJSONPath}": ${error}`,
				);
			}
		}
	}
	...
	public LoadEffect(effectName: string, js?: string, css?: string): void {
		if (this.IsEffectLoaded(effectName)) {
			this.Log(
				'error',
				`LoadEffect called with effect name "${effectName}" but an effect by this name is already currently loaded`,
			);
			return;
		}
		...
		if (js === undefined && css === undefined) {
			...
			if (!fs.existsSync(effectDir)) {
				this.Log(
					'error',
					`LoadEffect called without manually specifying JS/CSS for effect and effect name "${effectName}" does not have a directory at "${effectDir}"`,
				);
				return;
			}
			if (!fs.statSync(effectDir).isDirectory()) {
				this.Log(
					'error',
					`LoadEffect called without manually specifying JS/CSS for effect and effect name "${effectName}" when searched for in static effects directory "${effectDir}" was not a directory`,
				);
				return;
			}
			...
			if (!jsExists && !cssExists) {
				...
				this.Log(
					'error',
					`LoadEffect called without manually specifying JS/CSS for effect and effect name "${effectName}" when searched for in static effects directory "${effectDir}" did not have either a JS or CSS file at "${jsFilePath}" or "${cssFilePath}"`,
				);
				return;
			}
			...
			//and/or `css` variables should now be guaranteed to be defined
		}
		...
		this.loadedEffects.set(effectName, {
			...
			effectDisplayName:
				this.staticEffectConfigs.get(effectName)?.effectDisplayName ??
				`NO_EFFECT_DISPLAY_NAME_FOUND_FOR_${effectName}`,
			js: js,
			css: css,
			loadedAt: new Date(),
		});
	...
	public UnloadEffect(effectName: string): void {
		if (!this.IsEffectLoaded(effectName)) {
			this.Log(
				'error',
				`UnloadEffect called with effect name "${effectName}" but an effect by this name is not currently loaded`,
			);
			return;
		}
