...
export const vsBloomLogBrandNameColor: ColorDefinition = {
	rgb: [41, 184, 219],
	rgbFunc: colorful.rgb,
};
...
export const coloredLogSourceNameColors: Record<string, ColorDefinition> = {
	...
	Native: { rgb: [255, 165, 0], rgbFunc: colorful.rgb }, //rich orange
};
...
function SlerpColorDefinitions(
	...
	if (theta < 1e-5) {
		return a.map((val, i) => Math.round((1 - t) * val * 255 + t * b[i] * 255)) as [
			number,
			number,
			number,
		];
...
export function ConstructVSBloomLogPrefix(
	...
	const logBrandToSourceSepColor = SlerpColorDefinitions(
		logBrandNameColor,
		coloredLogSourceNameColors[source],
		0.5,
	);
	const logSourceColor = coloredLogSourceNameColors[source];

	if (logType) {
		const logSourceToTypeSepColor = SlerpColorDefinitions(
			logSourceColor,
			logTypeColoring[logType],
			0.5,
		);
...
export function ConstructNonBrandedLogPrefix(
	source: keyof typeof coloredLogSourceNameColors,
	logType?: keyof typeof logTypeColoring,
): string {
	const logSourceColor = coloredLogSourceNameColors[source];
