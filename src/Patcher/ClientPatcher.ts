 ...
 * subject(for good reason) so it's imperative to maintain
...
export const ALL_POSSIBLE_FILES_PATCHED_BY_VSBLOOM = [
	'workbench.html',
...
export async function GetMainApplicationProductFile(vscode: typeof import('vscode')) {
	...
	if (appProductFileCfg === 'auto') {
		const vsOutputDir = require.main
			? path.dirname(require.main.filename)
			: (globalThis as unknown as { _VSCODE_FILE_ROOT?: string })._VSCODE_FILE_ROOT;
		if (!vsOutputDir) {
			throw new Error(
				Common.RaiseError(
					"Unable to locate the output directory for the VSCode client; the extension cannot function without this information: Please provide the appropriate path for the 'product.json' file in the 'vsbloom.patcher.appProductFile' extension config to resolve this error.",
				),
			);
		}
		...
		if (!(await Common.IsThereADirectoryAtPath(mainVSAppDirectoryPath))) {
			throw new Error(
				Common.RaiseError(
					"Unable to locate the main application directory for the VSCode client automatically, the extension cannot function without this information: Please provide the appropriate path for the 'product.json' file in the 'vsbloom.patcher.appProductFile' extension config to resolve this error.",
				),
			);
		}
		...
		if (!(await Common.IsThereAFileAtPath(hopefullyProductFilePath))) {
			throw new Error(
				Common.RaiseError(
					"Unable to locate the 'product.json' file for the VSCode client automatically, the extension cannot function without this information: Please provide the appropriate path for the 'product.json' file in the 'vsbloom.patcher.appProductFile' extension config to resolve this error.",
				),
			);
		}
		if (!(await Common.CanIMessWithThisFile(hopefullyProductFilePath))) {
			throw new Error(
				Common.RaiseError(
					"The 'product.json' file associated with the VSCode client does not currently appear to be modifiable; depending on your operating system and VSC's installation path, you may need to re-launch the application as an administrator to allow this file to be modified by the VSBloom extension.",
				),
			);
		}
		...
		if (!fs.existsSync(appProductFileCfg)) {
			throw new Error(
				Common.RaiseError(
					`Invalid filesystem path provided for vsbloom.patcher.appProductFile - path does not exist: ${appProductFileCfg}`,
				),
			);
		}
		if (path.basename(appProductFileCfg) !== 'product.json') {
			throw new Error(
				Common.RaiseError(
					`Invalid filesystem path provided for vsbloom.patcher.appProductFile - path does not point to a 'product.json' file: ${appProductFileCfg}`,
				),
			);
		}
		if (!fs.statSync(appProductFileCfg).isFile()) {
			throw new Error(
				Common.RaiseError(
					`Invalid filesystem path provided for vsbloom.patcher.appProductFile - path is not a file`,
				),
			);
		}
		...
		throw new Error(
			Common.RaiseError(
				"Invalid value provided for vsbloom.patcher.appProductFile - expected 'auto' or a filesystem path",
			),
		);
	}
}

export async function GetFilePatchPathsForCurrentSettings(
	vscode: typeof import('vscode'),
): Promise<string[]> {
	const mainAppProductFilePath = await GetMainApplicationProductFile(vscode);
	const config = vscode.workspace.getConfiguration();
	const wantsClientCorruptWarningSupression = config.get<boolean>(
		'vsbloom.patcher.suppressCorruptionWarning',
	);
	...
	if (wantsClientCorruptWarningSupression) {
		filePatchPaths.push(
			await GetPathToAppFile(mainAppProductFilePath, 'workbench.desktop.main.js'),
		);
	}
...
export async function GetPathToAppFile(mainAppProductFilePath: string, fileName: string) {
	...
	try {
		...
		if (!checksums) {
			throw new Error(
				Common.RaiseError(
					`The main VSC application 'product.json' file is missing the 'checksums' field, unable to resolve paths to required product files.`,
				),
			);
		}
		...
		if (!fileKeyInProductJSON) {
			throw new Error(
				Common.RaiseError(
					`Attempt to resolve the path to the product file '${fileName}' failed, the file does not seem to be currently tracked in the main VSC application's 'product.json' file.`,
				),
			);
		}
		...
		throw new Error(
			Common.RaiseError(
				`Unable to parse the main VSC application 'product.json' file, is the file corrupted or otherwise invalid JSON?`,
			),
		);
	}
}
...
export async function IsElectronHTMLFilePatched(initFilePath: string): Promise<boolean> {
	if (!(await Common.IsThereAFileAtPath(initFilePath))) {
		throw new Error(
			Common.RaiseError(
				`The Electron init file at '${initFilePath}' does not exist or is not a file.`,
			),
		);
	}
	if (!(await Common.CanIMessWithThisFile(initFilePath))) {
		throw new Error(
			Common.RaiseError(
				`The Electron init file at '${initFilePath}' is not accessible; the extension likely lacks permissions to read/write to it.`,
			),
		);
	}

	return fs.promises
		.readFile(initFilePath, 'utf8')
		.then((content) => {
		...
		.catch((err) => {
			throw new Error(
				Common.RaiseError(
					`Unable to read the contents of the Electron init file at '${initFilePath}'\n\nError: ${err.message}`,
				),
			);
		});
}
...
export async function IsElectronJSFilePatched(jsFilePath: string): Promise<boolean> {
	if (!(await Common.IsThereAFileAtPath(jsFilePath))) {
		throw new Error(
			Common.RaiseError(
				`The Electron JS file at '${jsFilePath}' does not exist or is not a file.`,
			),
		);
	}
	if (!(await Common.CanIMessWithThisFile(jsFilePath))) {
		throw new Error(
			Common.RaiseError(
				`The Electron JS file at '${jsFilePath}' is not accessible; the extension likely lacks permissions to read/write to it.`,
			),
		);
	}
	...
	return fs.promises
		...
		.catch((err) => {
			throw new Error(
				Common.RaiseError(
					`Unable to read the contents of the Electron JS file at '${jsFilePath}'\n\nError: ${err.message}`,
				),
			);
		});
}
...
export async function UpdateChecksumForTrackedFile(
	...
	return applicationProductFileMutex.runExclusive(async () => {
		...
		try {
			...
			if (!checksums) {
				throw new Error(
					Common.RaiseError(
						`The main VSC application 'product.json' file is missing the 'checksums' field, thus we cannot update internal VSC file checksums: This likely means that VSCode has updated their product.json format and VSBloom is out of date.`,
					),
				);
			}

			const fileName = path.basename(filePath);
			const fileKeyInProductJSON = Object.keys(checksums).find((key) =>
				key.includes(fileName),
			);
			if (!fileKeyInProductJSON) {
				throw new Error(
					Common.RaiseError(
						`Attempt to update the VSC product checksum tracking for a file which does not seem to be currently tracked: '${filePath}'`,
					),
				);
			}
			...
			await fs.promises
				.writeFile(mainAppProductFilePath, JSON.stringify(productJSON, null, '\t'))
				.catch((err) => {
					throw new Error(
						Common.RaiseError(
							`Unable to save the main VSC application 'product.json' file after updating a tracked file's checksum: ${err.message}`,
						),
					);
				});
		} catch (_err) {
...
export async function GetClientLauncherScriptElementString(
	...
	if (!(await Common.IsThereAFileAtPath(clientScriptPath))) {
		throw new Error(
			Common.RaiseError(
				`VSBloom Client script not found at '${clientScriptPath}'. Please ensure the extension is properly built.`,
			),
		);
	}
...
function PatchElectronWorkbenchCSPElement(wbHtmlSource: string) {
	...
	const cspMetaElementExtractionRegex =
	...
	if (scriptSrcMatch) {
		...
		//second match contains all of the allowed sources for the script-src directive (note though that this doesn't contain the semicolon to end the directive)
		...
		if (!scriptSrcSources.includes('unsafe-inline')) {
			directivesToPatchIntoScriptSrcDirective.push(
				`${quoteUsedForScriptSrcSources}unsafe-inline${quoteUsedForScriptSrcSources}`,
			);
		}
		...
		if (directivesToPatchIntoScriptSrcDirective.length > 0) {
			...
			//now we need to reconstruct the script-src directive in the actual CSP content
	...
	if (cspContent !== unmodifiedCSPContent) {
		...
		const unmodifiedFullCSPElement = wbHtmlSource.substring(
			cspElementMatch.index,
			cspElementMatch.index + cspElementMatch[0].length,
		);

		const patchedFullCSPElement = unmodifiedFullCSPElement.replace(
			unmodifiedCSPContent,
			cspContent,
		);

		const patchedHTMLSrc = wbHtmlSource.replace(
			unmodifiedFullCSPElement,
			patchedFullCSPElement,
		);
...
export async function PatchElectronHTMLFile(
	...
	if (fileContents.includes(HTML_FILE_PATCH_INDICATOR)) {
		throw new Error(
			Common.RaiseError(
				`Attempt to patch Electron init file at '${initFilePath}', but it was already patched(?)`,
			),
		);
	}
	...
	patchedFileContents =
		patchedFileContents.slice(0, headTagEndIdx) +
		`${sharedLibsPayload}\n${clientPayload}\n` +
	...
	await fs.promises.writeFile(initFilePath, patchedFileContents).catch((err) => {
		throw new Error(
			Common.RaiseError(
				`Unable to save patches applied to VSC's Electron init file at '${initFilePath}': ${err.message}`,
			),
		);
	});
...
//and we don't 100% *have* to disable it, but it's rather bad UX
...
//than happy to accommodate and address your concerns in a manner
...
export async function SuppressWorkbenchClientModificationWarning(
	...
	await fs.promises.writeFile(wbScriptPath, patchedWbScriptContents).catch((err) => {
		throw new Error(
			Common.RaiseError(
				`Unable to save patches applied to VSC's Electron JS file at '${wbScriptPath}': ${err.message}`,
			),
		);
	});
...
export async function WouldClientPatchingRequireElevation(
	mainAppProductFilePath: string,
): Promise<boolean> {
...
export async function PerformClientPatching(
	...
	await GetPathToAppFile(mainAppProductFilePath, 'workbench.html')
		...
		.catch((err) => {
			throw new Error(
				Common.RaiseError(
					`VSBloom's client patching process encountered an error during the HTML phase: ${err.message}`,
				),
			);
		});
	...
	if (wantsClientCorruptWarningSupression) {
		...
		await GetPathToAppFile(mainAppProductFilePath, 'workbench.desktop.main.js')
			...
			.catch((err) => {
				throw new Error(
					Common.RaiseError(
						`VSBloom's client patching process encountered an error during the JS phase: ${err.message}`,
					),
				);
			});
	}
...
export async function UnPatchClient(mainAppProductFilePath: string) {
	await GetPathToAppFile(mainAppProductFilePath, 'workbench.html')
		.then(async (htmlPath) => {
			...
			if (isWorkbenchHTMLPatched) {
				...
				if (!hasBackup) {
					throw new Error(
						Common.RaiseError(
							`Attempt to un-patch Electron HTML file at '${htmlPath}' failed, no backup file found(?)`,
						),
					);
				}
		...
		.catch((err) => {
			throw new Error(
				Common.RaiseError(
					`VSBloom's client un-patching process encountered an error during the HTML phase: ${err.message}`,
				),
			);
		});
		...
		.then(async (wbJsPath) => {
			...
			if (isWorkbenchJSPatched) {
				...
				if (!hasBackup) {
					throw new Error(
						Common.RaiseError(
							`Attempt to un-Patch Electron JS file at '${wbJsPath}' failed, no backup file found(?)`,
						),
					);
				}
