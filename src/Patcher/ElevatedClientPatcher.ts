...
if (args.length < 2) {
	console.error(
		"[VSBloom]: ElevatedClientPatcher's process was not provided with the action to perform and the path to the main application product file as arguments(?)",
	);
	process.exit(1);
}
...
if (action === 'patch') {
	...
	if (args.length < 6) {
		console.error(
			"[VSBloom]: ElevatedClientPatcher's patch action requires: suppressCorruptWarning, bridgePort, authToken, and extensionPath arguments",
		);
		process.exit(1);
	}
	...
	if (Number.isNaN(bridgePort)) {
		console.error(
			"[VSBloom]: ElevatedClientPatcher's bridgePort argument is not a valid number",
		);
		process.exit(1);
	}
	...
	ClientPatcher.PerformClientPatching(
		...
		.catch((err) => {
			console.error(
				"[VSBloom]: ElevatedClientPatcher's process encountered an error during the client patching process: ",
				err,
			);
			process.exit(1);
		});
} else if (action === 'unPatch') {
	ClientPatcher.UnPatchClient(mainAppProductFilePath)
		.then(() => {
		...
		.catch((err) => {
			console.error(
				"[VSBloom]: ElevatedClientPatcher's process encountered an error during the client un-patching process: ",
				err,
			);
			process.exit(1);
		});
	...
	console.error(
		"[VSBloom]: ElevatedClientPatcher's process was not provided with a valid action to perform(?)",
	);
	process.exit(1);
}
