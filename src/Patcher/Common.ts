			...
			fs.promises
				...
				.then(() =>
					fs.promises.stat(filePath).then((stat) => {
						...
						return action(filePath).catch((err) => {
							RaiseWarning(
...
export async function CanIMessWithThisFile(filePath: string): Promise<boolean> {
	return fs.promises
		...
		.then((stat) => {
			...
			return fs.promises
				.access(filePath, fs.constants.F_OK | fs.constants.R_OK | fs.constants.W_OK)
				.then(() => true)
				.catch(() => false);
		})
		.catch(() => {
...
export async function GetFileChecksum(filePath: string): Promise<string> {
	return fs.promises
		...
		.catch((err) => {
			throw new Error(
				RaiseError(
					`Unable to read the contents of the file at '${filePath}' when attempting to generate its checksum: ${err.message}`,
				),
			);
		});
}
