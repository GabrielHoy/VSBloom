...
export function GetBackupFilePathFor(nonBackupFilePath: string): string {
	return path.join(
		path.dirname(nonBackupFilePath),
		`${path.basename(nonBackupFilePath)}.bak${Common.VSBLOOM_FILE_EXTENSION}`,
	);
}
...
export async function DoesFileHaveBackup(nonBackupFilePath: string) {
	return fs.promises
		.stat(GetBackupFilePathFor(nonBackupFilePath))
		.then((statData) => statData.isFile())
		.catch(() => false);
}
...
export async function RestoreFileFromBackup(
	nonBackupFilePath: string,
	removeBackupOnceDone: boolean,
) {
	const backupFilePath = GetBackupFilePathFor(nonBackupFilePath);
