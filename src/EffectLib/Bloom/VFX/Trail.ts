...
export class Trail {
	...
	public SnapHeadTo(
		pos: Point,
		shouldUpdateGoal: boolean = true,
		moveExistingHeadInsteadOfAddingNewPoint: boolean = false,
	): void {
		if (shouldUpdateGoal) {
	...
	protected MoveHeadTowardsGoal(goalPos: Point = this.goal): boolean {
		...
		const headMovementThisFrame = curHeadPosToGoal
			.normalize()
			.multiply(this.headSpeed.multiplyScalar(this.app.ticker.deltaTime));
		...
		const currentSegmentBeginning: Point | undefined =
			this.trailGeometry[this.trailGeometry.length - 2];
		...
		//need to normalize this delta to -pi/pi since either curSegAngle or unconstrainedNewSegmentAngle
		...
		if (Math.abs(segmentAngleDelta) > maxSegmentAngleDeltaConstraint) {
			...
			if (this.proximityToGoalWhenAngularlyConstrainedForSlowdown > 0) {
				const proximityMultiplier = Math.min(
					1,
					curHeadPosToGoal.magnitude() /
						this.proximityToGoalWhenAngularlyConstrainedForSlowdown,
				);
				constrainedNewSegmentVec.multiplyScalar(
					proximityMultiplier,
					constrainedNewSegmentVec,
				);
			}
			...
			if (progressTowardGoal > 0) {
				...
				if (movementMag > progressTowardGoal) {
					constrainedNewSegmentVec = constrainedNewSegmentVec
						.normalize()
						.multiplyScalar(progressTowardGoal);
				}
			}
	...
	protected ShortenTail(): boolean {
		...
		while (distToShorten > 0) {
			...
			if (segmentLength <= distToShorten) {
				...
				const amountToMoveTrailTailPoint: Point = segmentVec
					.normalize()
					.multiplyScalar(distToShorten);
	...
	protected UpdateTrail() {
		...
		if (this.trailGeometry.length > 1) {
			...
			updatedTrailGeometryThisFrame =
				updatedTrailGeometryThisFrame || didTailShorteningAffectGeometry;
		}
	...
	constructor(
		...
		ropeOptions: Omit<MeshRopeOptions, 'points'>,
	) {
	...
	public destroy(
		...
		if (this.rope) {
			//rope is readonly *but* pixi.js can remove descendants if parent gets removed
