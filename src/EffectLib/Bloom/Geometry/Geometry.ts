...
export function GetAngleBetweenVectorsRad(v1: Point, v2: Point): number {
	if (v1.magnitudeSquared() === 0 || v2.magnitudeSquared() === 0) {
		throw new Error(
			`Attempt to get the angle between two vectors where vector ${v1.magnitudeSquared() === 0 ? '(v1)' : '(v2)'} is a zero vector: This would result in a division by zero/NaN value.`,
		);
	}
