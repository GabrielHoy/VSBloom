...
const libs = (window as unknown as { __VSBLOOM__?: { libs: { [key: string]: unknown } } })
	.__VSBLOOM__?.libs;

if (!libs?.gsap) {
	throw new Error(
		"[VSBloom]: GSAP was not available, VSBloom's SharedLibraries may not have loaded correctly.",
	);
}
