...
namespace VSBloom::Audio {
    ...
    void AudioAnalyzer::RunAnalysis(AnalyzedAudioFrame& outFrame) {
        ...
        std::array<float, ANALYSIS_FFT_BIN_COUNT> rawBins;
    ...
    void AudioAnalyzer::MapMagnitudesToLogBins(std::array<float, ANALYSIS_FFT_BIN_COUNT>& outBins) const {
        ...
        constexpr float maxFrequencyHz = 22'500.0f;
        // Display range for the dB-scaled output bins
        constexpr float dbFloor   = -50.0f;
        constexpr float dbCeiling = 0.0f;
        const float     hzPerBin  = static_cast<float>(ANALYSIS_SAMPLE_RATE) / static_cast<float>(FFT_FRAME_SIZE);
        ...
        for (std::size_t band = 0; band < ANALYSIS_FFT_BIN_COUNT; ++band) {
            ...
            const float fracLow  = std::clamp(freqLow / hzPerBin, 1.0f, maxUsableIdx);
            const float fracHigh = std::clamp(freqHigh / hzPerBin, fracLow, maxUsableIdx);
